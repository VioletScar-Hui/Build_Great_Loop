...
| STATE | Work crosses runs, retries can duplicate effects, or interruption recovery is promised | CORE | Kill after claim and after effect; restart neither loses nor duplicates work |
...
| FLOW | Large/replayable fan-out or branching would flood the main context | CORE; normally STATE | Executable orchestration persists branches/intermediates; main context receives compact results |
| CONTAIN | L2/L3 or access to consequential systems/data | CORE | Environment policy blocks a seeded forbidden action even when the prompt asks for it |

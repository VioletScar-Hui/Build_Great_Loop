...
- **"最小套件/按需/起步/as-needed" 当范围边界** → 打回。这类措辞不可机器核对，
  跨会话邀请 scope creep（真实复盘：一个「防风控最小套件起步」的 loop 分 3+4+2+2
  波续补，把范围从「最小」漂到对齐参照系统 ~12/22 项，无人喊停）。范围边界必须
  **枚举**：「就这 N 项：<清单>；超出清单的任何新增 = 新 loop 或 human gate」。
  「按需再加」不是边界，是没有边界。

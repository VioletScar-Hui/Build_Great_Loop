...
| 技能 | 当前版本 | 作用 | 什么时候触发 |
|---|---:|---|---|
| **loop-spec** | 4.1.0 | 访谈式把模糊任务变成一份可执行的 `SPEC.md` | 任务还没想清、需要先定目标/成功标准/停止条件 |
| **loop-engineering**（核心） | 7.1.0 | 访谈 → 产出**可粘贴的循环提示词本体** | 「帮我写个能一直跑到完成的 agent / 循环 / harness」 |
| **loop-eval** | 3.1.0 | 设计成功标准 + 小评测集 + 评分器（pass@k vs pass^k） | 「怎么知道这循环靠不靠谱 / 给 agent 写评测」 |
| **loop-review** | 4.1.0 | 体检并加固一份**已有**的循环提示词 | 「我的 agent 老是停不下来 / 说完成了其实没有 / 重启丢进度」 |
| **loop-ops** | 3.1.0 | 把循环跑成周期性/无人值守的自动化（调度·成本·安全门·分级放权） | 「每天/每个 PR/每晚跑这个 agent」「无人值守怎么跑不出事」「babysit 我的 PR」「自动 triage issue」 |
| **loop-retro** | 4.1.0 | 跑完复盘：证据引证诊断 → harness 修订提案 + 真实失败 gotcha + 标准提案 + **沉淀判定** | 「loop 跑完了帮我复盘」「看看 run log 哪里出问题」「为什么超预算/thrash」 |
...
runtime/authority 合约 16/16、suite validator 7/7，另有 6 组中英行为不变量配对用例。
配对模型评测为候选 31/33、旧基线 27/33；
...
| 2026-07-22 v4.1 | **吸收 anthropic-mind v1.7 的运行时契约**：显式区分只读并行与副作用串行；状态、幂等键和恢复绑定 tenant/channel/principal/connector/memory/permission；评测固定控制接口；加入宿主中立的模型/effort 适配层、Claude W29 `/fork` 与 `/subtask` 语义、当前 Claude assistant prefill 拒绝规则，以及 6 组中英行为不变量。原 README 内容完整保留。 |
...
│   ├── SKILL.md
│   ├── references/          # 原理、模式、模板、运行时适配、组件目录与审计清单
│   ├── assets/
│   │   ├── harness-core.md / harness-skeleton.md
│   │   └── components/      # 按触发条件装配的可选组件
│   ├── evals/               # 示例评测集、路由集、对抗与回归 fixtures
│   └── scripts/             # 组件选择、状态、runtime contract、containment、quota 与 suite 验证器
